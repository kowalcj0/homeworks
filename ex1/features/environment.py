...
def before_all(context):
    # API URL under test
    context.platform_url = 'https://api.stackexchange.com'
    context.test_url = '{}{}'.format(
        context.platform_url,
        '/2.2/search?order=desc&sort=activity&site=stackoverflow&intitle=')
    ...
    # Check if the Stack Exchange API is reachable
    try:
        ...
        raise Exception(
            ('Unable to connect to the test URL {} - please ensure that '
             'Stack Exchange API is reachable'.format(context.platform_url)))
